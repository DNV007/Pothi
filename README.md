...
- Optional linked folder sync (on open, hourly, and via **Sync** button)
...
After import, a summary toast reports how many entries were found, imported, skipped as duplicates, citekey-renamed (e.g., `Smith2024→Smith2024a`), or unreadable.
...
- Linked manuscript folders receive `references.bib` and `refs.json` automatically on open and once per hour. Click **Sync** in the manuscript header to write immediately. A "saved HH:MM" pill confirms the last successful write; "save failed" appears if the write could not be verified.
...
After linking, `references.bib` and `refs.json` are written once immediately, then once per hour. Use the **Sync** button in the manuscript header to trigger a write at any time. After each write, Pothi reads back the file size to confirm the bytes landed; a "saved HH:MM" indicator shows the last verified write time.

If folder permission expires:

1. Click **Resume** in the sidebar.
2. Grant read/write access again.
3. Pothi resumes writing on the next Sync or scheduled hour.
...
Network requests to CrossRef, OpenAlex, and OpenLibrary time out after 15 seconds. Enrichment requests (abstract, citation count) time out after 8 seconds and fail silently so the primary record is still saved.
...
2. Folder permission is still granted (click **Resume** in the sidebar if it has lapsed).
3. Manuscript has a linked folder.
4. The entry is cited in that manuscript.
5. Click **Sync** in the manuscript header to trigger an immediate write and confirm with the "saved HH:MM" indicator.

If the indicator shows "save failed", hover it for the error detail (quota exceeded, permission revoked, or a network-mounted folder that went offline).
...
[img-new-reference]: docs/manual-assets/01-new-reference-modal.png
[img-library-detail]: docs/manual-assets/02-library-detail.png
[img-manuscript]: docs/manual-assets/03-manuscript-view.png
[img-search]: docs/manual-assets/04-search-modal.png
