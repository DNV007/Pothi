...
// Hard timeout for every outbound fetch. Without it a hung CrossRef /
// OpenAlex endpoint can stall the add-reference and search flows
// indefinitely. Primary lookups get a generous window; the enrichment
// hooks (OpenAlex / S2 inside fetchDoiMetadata) use a tighter one since
// they soft-fail on null.
const PRIMARY_TIMEOUT_MS = 15000;
const ENRICH_TIMEOUT_MS  = 8000;

function fetchWithTimeout(url, options = {}, timeoutMs = PRIMARY_TIMEOUT_MS) {
...
export async function fetchDoiMetadata(doi) {
  ...
  try {
    ...
    if (isAbortError(e)) throw new Error('CrossRef: request timed out');
...
export async function fetchOpenAlexMeta(doi) {
  ...
  let res;
  try { res = await fetchWithTimeout(url, {}, ENRICH_TIMEOUT_MS); }
  catch (_) { return null; }
...
export async function searchWeb(query, limit = 25, mode = 'auto') {
  const q = String(query || '').trim();
  if (!q) return [];
  const doi = extractDoiFromText(q);
  if (doi) {
    const entry = await fetchDoiMetadata(doi);
    const result = entryToSearchResult(entry, 'doi');
    const out = [result];
    out._resolvedMode = 'doi';
    return out;
  }
...
function entryToSearchResult(entry, source) {
  const f = entry.fields || {};
  return {
    title: f.title || '',
    authors: f.author || f.editor || '',
    year: f.year ? Number(f.year) : null,
    venue: f.journal || f.booktitle || f.publisher || f.school || '',
    doi: f.doi || null,
    abstract: f.abstract || '',
    citationCount: f.citationCount ? Number(f.citationCount) : null,
    influentialCitationCount: f.influentialCitationCount ? Number(f.influentialCitationCount) : null,
    openAccessUrl: f.url || null,
...
async function searchOpenAlex(query, limit, mode = 'anywhere') {
  ...
  try {
    ...
    if (isAbortError(e)) throw new Error('OpenAlex: request timed out');
    throw e;
  }
...
async function searchSemanticScholar(query, limit) {
  ...
  try {
    ...
    if (isAbortError(e)) throw new Error('Semantic Scholar: request timed out');
    throw e;
  }
...
export async function fetchIsbnMetadata(rawIsbn) {
  ...
  try { res = await fetchWithTimeout(url); }
    ...
    if (isAbortError(e)) throw new Error('OpenLibrary: request timed out');
    throw new Error('Network error: ' + e.message);
  }
